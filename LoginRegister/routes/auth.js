...
router.get("/getDoctors", (req, res) => {
  ...
  db.query(sql, (err, results) => {
    if (err) {
      ...
      res
        .status(500)
        .json({ message: "Error fetching doctors data", error: err });
    } else {
...
router.post("/DoctorLogin", (req, res) => {
  ...
  if (!doctor_id || !password) {
    return res
      .status(400)
      .json({ message: "Please enter both Doctor ID and password." });
  }
  ...
  db.query(sql, [doctor_id], (err, results) => {
    ...
    res
      ...
      .json({
        ...
        redirect: "/DoctorsDashboard.html",
      });
  });
});
...
router.get("/getDoctorDetails/:id", (req, res) => {
  ...
  const sql =
    "SELECT first_name, last_name, doctor_specialty FROM DoctorsDetails WHERE doctor_id = ?";
...
router.get("/getDoctorAppointments", (req, res) => {
  ...
  const sql =
    "SELECT * FROM patients_details WHERE doctor_id = ? AND status = 'pending' ORDER BY appointment_date ASC";
  db.query(sql, [doctorId], (err, results) => {
    if (err) {
...
router.get("/getBookedAppointments", (req, res) => {
  ...
  const sql =
    "SELECT * FROM patients_details WHERE doctor_id = ? AND status = 'accepted'";
  db.query(sql, [doctorId], (err, results) => {
    if (err) {
...
router.get("/getDoctorsLeave", (req, res) => {
  const sql =
    "SELECT id, doctor_id, doctor_name, doctor_specialty, DATE_FORMAT(leave_date, '%d-%m-%Y') AS formatted_date FROM doctorattendance ORDER BY leave_date DESC";
...
router.get("/getDoctorLeave", (req, res) => {
  ...
  const sql =
    "SELECT doctor_id, doctor_name, doctor_specialty, DATE_FORMAT(leave_date, '%d-%m-%Y') AS formatted_date FROM doctorattendance WHERE doctor_id = ? ORDER BY leave_date DESC";
